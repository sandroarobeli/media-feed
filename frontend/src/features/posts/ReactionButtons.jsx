...
const ReactionButtons = (props) => {
    ...
    const reactionButtons = Object.entries(reactionEmoji).map(([name, emoji]) => {
        return (
            ...
            >
                {emoji} <span style={{ color: currentTheme ? "black" : "white", marginLeft: '5px' }}>{props.post.reactions[name]}</span>
          </Button>
        )
      })
    
      return <Stack  direction="row" spacing={0} sx={{marginLeft: '-0.5rem'}}>{reactionButtons}</Stack>
}
